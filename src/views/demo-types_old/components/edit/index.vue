        ...
        <v-row>
          ...
          <v-col cols="12" md="6">
            <new-image-upload
              ...
              :imgUrl="currItem.template ? currItem.template.path : ''"
  ...
  methods: {
    ...
    submit() {
      ...
      if (this.template) {
        ...
        UpdateMedia({ id: this.currItem.template.id, data: payload })
          .then(() => {
            this.reset();
            this.$emit("closed");
            this.loading.submit = false;
    ...
    handleLocaleChange(locale) {
      ...
      ShowData({ reqName: "demo-types", id: this.currItem.id, locale })
        .then(res => {
          this.locale = locale;
